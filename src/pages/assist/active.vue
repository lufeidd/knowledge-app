<template>
  <div id="activePage" :style="{'background-color':bgcolor}">
    ...
    <div class="startHelpTwo">
      <div class="startHelp" v-if="activityData.base.launch_id" @click="billShow">
        邀请好友助力
    ...
    <div class="helpFriend" v-if="activityData.base.launch_id">
      <div class="helpAndPic">
        <div class="helpCount" :style="{'color':color}">
          ...
          >{{ activityData.launcher.curr_num }}人</span>
          <span>助力</span>
        </div>
        <div class="helpPicture" @click="pictureShow">
          <div class="oval" v-if="activityData.launcher.supporter_list.length > 0">
            <div
              class="bookImg"
              v-lazy:background-image="activityData.launcher.supporter_list[0].user_pic"
            ></div>
          </div>
          <div
            class="oval"
            style="right: 33px;"
            v-if="activityData.launcher.supporter_list.length > 1"
          >
            <div
              class="bookImg"
              v-lazy:background-image="activityData.launcher.supporter_list[1].user_pic"
            ></div>
          </div>
          <div
            class="oval"
            style="right: 46px;"
            v-if="activityData.launcher.supporter_list.length > 2"
          >
            <div
              class="bookImg"
              v-lazy:background-image="activityData.launcher.supporter_list[2].user_pic"
            ></div>
          </div>
    ...
    <div class="rewardContent">
      <div v-for="(item,index) in activityData.base.reward_list" :key="index">
        <div
          class="bookImg"
          v-lazy:background-image="item.ad_pic"
          @click="receiveAwardes(item,index)"
        >
          <div class="callBack">
      ...
      <div class="addressBox">
        <div v-for="(Item,index) in addressData" :key="index">
          <div class="addressMember">
            ...
            <div
              class="title"
            >{{ Item.consignee }}/{{ Item.mobile }}/{{ Item.province }}{{ Item.city }}{{ Item.county }}{{ Item.address }}</div>
          </div>
        </div>
      </div>
      <div class="button">
...
import {
  ASSISTANCE_DETAIL,
  ASSISTANCE_POSTER,
  ASSISTANCE_SUPPORTER,
  ASSISTANCE_TAKEPRIZE
} from "../../apis/assist";
  ...
  data() {
    return {
      ...
      activeList: [
        {
          pic:
            "http://file.mhuoba.com/picture/shopadmin/20190705/11/20190705111229706.jpg"
        }
      ]
    };
  },
